...
unsigned long CMD_GAP_US = 25;    // Задержка между командами (в микросекундах)
...
CommandType currentCommand = CMD_FORWARD;  // Начинаем с команды "вперёд"
unsigned long lastCommandTime = 0;         // Время последней отправки команды
unsigned long COMMAND_INTERVAL = 50;       // Интервал между командами в миллисекундах (это видно на осциллографе)
...
void setup() {
  ...
  Serial.println("L[value] - set CMD_GAP_US (e.g., L25)");
  Serial.println("I[value] - set COMMAND_INTERVAL in ms (e.g., I100 for 100ms)");
  Serial.println("P - toggle parity");
  Serial.println("I - Info");
  Serial.println("");
  Serial.println("Current values:");
  Serial.println("SYMBOL_GAP_US=" + String(SYMBOL_GAP_US) + "us, CMD_GAP_US=" + String(CMD_GAP_US) + "us, COMMAND_INTERVAL=" + String(COMMAND_INTERVAL) + "ms, Parity=" + String(even_parity ? "Even" : "Odd"));
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (command.length() > 0) {
      ...
      else if ((cmd == 'L' || cmd == 'l') && command.length() > 1) {
        unsigned long new_value = command.substring(1).toInt();
        CMD_GAP_US = new_value;
        Serial.println("CMD_GAP_US set to: " + String(CMD_GAP_US) + " us");
        commandProcessed = true;
      }
      ...
      if (commandProcessed) {
        if (cmd != 'I' || (command.length() == 1 && (cmd == 'I' || cmd == 'i'))) {  // Не выводим при команде I с параметром
          if (cmd != 'I' && cmd != 'i') {
            Serial.println("Current values: SYMBOL_GAP_US=" + String(SYMBOL_GAP_US) + "us, CMD_GAP_US=" + String(CMD_GAP_US) + "us, COMMAND_INTERVAL=" + String(COMMAND_INTERVAL) + "ms, Parity=" + String(even_parity ? "Even" : "Odd"));
            Serial.println("Current command: " + getCommandName(currentCommand));
          }
        }
      } else {
